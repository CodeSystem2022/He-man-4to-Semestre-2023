...
while opcion != 5:
    ...
    if opcion == 1:
        ...
        usuario = Usuarios(username=username_var, password=password_var)
        ...
        usuario = Usuarios(id_usuario=id_usuario_var,username=username_var, password=password_var)
        usuario_actualizado = UsuarioDAO.actualizar(usuario)
        log.info(f'Usuario actualizado: {usuario_actualizado}')

    elif opcion == 4:
        id_usuario_var = int(input('Digite el id del usuario a eliminar'))
        usuario = Usuarios(id_usuario=id_usuario_var)
